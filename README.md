...
<p align="center">
  <a target="_blank" href="https://github.com/almasud/augmented_learn/raw/refs/heads/master/app/release/app-release.apk">
...
The Augmented Learn application is developed to provide interactive learning
with Augmented Reality for preschool children. Our application is AR-optional which means it is designed for both non-AR and AR-supported devices.
...
### Learn:
In this section, the app introduces (teaches) preschool children some basic subjects (i.e. স্বরবর্ণ, ব্যঞ্জনবর্ণ, সংখ্যা, Alphabets, Numbers and Animals) by playing each item name and
showing the associated picture with the name (If necessary) one by one by pressing a next/previous button. For every item of every learn there is an available AR view
...
on the number of Test items. Each test page holds four items to choose the correct one by playing a voice that needs to choose. The examinee gets a wrong warning if the
clicked item is not correct. After clicking a correct one the test page proceeds to the next one. The process is continuing until the rest of the items are completed. All the wrong and
correct answers are tracked to make a Test result.
...
to render a single or multiple 3D models for each scanned item on top of it. Note that this feature works only for AR supported devices.
